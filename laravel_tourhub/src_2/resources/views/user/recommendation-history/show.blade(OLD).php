<x-layouts.tourhub-auth title="Detail Riwayat - TourHub Bali">
    ...
    <div class="space-y-6">
        {{-- Header --}}
        <section
            class="relative overflow-hidden rounded-3xl border border-slate-200 bg-slate-950 p-6 text-white shadow-2xl shadow-slate-900/10 md:p-8"
        >
            <div
                class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.40),_transparent_34%),radial-gradient(circle_at_bottom_right,_rgba(245,158,11,0.30),_transparent_34%)]"
            ></div>
            ...
            <div class="relative flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                <div class="max-w-3xl">
                    <div
                        class="inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-black text-blue-100 ring-1 ring-white/10"
                    >
                    ...
                    <p class="mt-3 text-sm leading-6 text-slate-300">
                        Halaman ini menampilkan detail request rekomendasi, hasil response FastAPI, dan destinasi yang
                        paling direkomendasikan berdasarkan
        ...
        <section class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-3">
                    <span
                        class="{{ $log->status === 'success' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }} inline-flex rounded-full px-3 py-1 text-sm font-black"
                    >
                        {{ ucfirst($log->status) }}
                    </span>
                </div>

                <p class="mt-4 text-xs text-slate-500">Status request rekomendasi.</p>
            </div>
            ...
            <div class="rounded-3xl border border-blue-200 bg-white p-5 shadow-sm">
                ...
                <p class="mt-1 text-xs text-slate-500">Source: {{ $log->weather_source ?? 'manual' }}</p>
            </div>
            ...
            <div class="rounded-3xl border border-emerald-200 bg-white p-5 shadow-sm">
                ...
                <p class="mt-1 text-xs text-slate-500">Jumlah kandidat dari FastAPI.</p>
            </div>
            ...
            <div class="rounded-3xl border border-amber-200 bg-white p-5 shadow-sm">
                ...
                <p class="mt-1 text-xs text-slate-500">Waktu respons rekomendasi.</p>
            </div>
        </section>
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                    <p class="text-sm font-black tracking-wider text-slate-500 uppercase">Parameter Request</p>

                    <h2 class="mt-1 text-2xl font-black text-slate-950">Preferensi yang Digunakan</h2>

                    <p class="mt-2 text-sm text-slate-600">Ringkasan input user saat rekomendasi ini dibuat.</p>
                </div>
            ...
            <div class="mt-6 grid grid-cols-1 gap-3 md:grid-cols-4">
                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Kabupaten/Kota</p>
                    <p class="mt-1 font-black text-slate-950">{{ $kabupatenKota ?: '-' }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Kecamatan</p>
                    <p class="mt-1 font-black text-slate-950">{{ $kecamatan ?: '-' }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Min Rating</p>
                    <p class="mt-1 font-black text-slate-950">{{ $minRating }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Top N</p>
                    <p class="mt-1 font-black text-slate-950">{{ $topN }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Hari Kunjungan</p>
                    <p class="mt-1 font-black text-slate-950">{{ ucfirst((string) $visitDay) }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">BMKG</p>
                    <p class="mt-1 font-black text-slate-950">{{ $useBmkg }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">High Season</p>
                    <p class="mt-1 font-black text-slate-950">{{ $isHighSeason }}</p>
                </div>

                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Keywords</p>
        ...
        @if ($recommendations->isNotEmpty())
            {{-- Best Recommendation --}}
            <section
                class="overflow-hidden rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-blue-50 shadow-xl shadow-amber-900/5"
            >
                <div class="grid grid-cols-1 lg:grid-cols-12">
                    <div class="relative lg:col-span-5">
                        ...
                        <div class="absolute right-4 bottom-4 left-4 rounded-3xl bg-white/90 p-4 backdrop-blur-xl">
                    ...
                    <div class="p-6 md:p-8 lg:col-span-7">
                        ...
                        <div class="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
                            ...
                            <div class="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
                                <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Jumlah Rating</p>
                                <p class="mt-1 text-xl font-black text-slate-950">
                            ...
                            <div class="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
                                <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">CBF</p>
                                <p class="mt-1 text-xl font-black text-slate-950">
                            ...
                            <div class="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
                                <p class="text-xs font-bold tracking-wide text-slate-500 uppercase">Context</p>
                                <p class="mt-1 text-xl font-black text-slate-950">
                        ...
                        <div class="mt-6 rounded-2xl border border-amber-200 bg-white/80 p-4">
                            ...
                            <p class="mt-2 text-sm leading-6 text-slate-700">
                                ...
                                dibanding kandidat lain pada riwayat ini. Final score digunakan sebagai skor ranking
                                akhir dari kombinasi CBF, rating, jumlah ulasan, dan penyesuaian konteks CARS.
                            </p>
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    <div>
                        <p class="text-sm font-black tracking-wider text-slate-500 uppercase">Ranking Rekomendasi</p>

                        <h2 class="text-2xl font-black text-slate-950">Diurutkan Berdasarkan Final Score</h2>
                ...
                <div class="space-y-4">
                    @foreach ($recommendations as $index => $item)
                        <article
                            class="{{ $index === 0 ? 'border-amber-300 bg-amber-50/40' : 'border-slate-200 bg-white' }} overflow-hidden rounded-3xl border"
                        >
                            <div class="flex flex-col gap-4 p-4 md:flex-row">
                                <div class="relative shrink-0">
                                    ...
                                    @else
                                        <div
                                            class="flex h-40 w-full items-center justify-center rounded-2xl bg-slate-100 text-sm font-bold text-slate-400 md:w-52"
                                        >
                                            No Image
                                        </div>
                                    @endif
                                    ...
                                    <div
                                        class="{{ $index === 0 ? 'bg-amber-400 text-slate-950' : 'bg-slate-950 text-white' }} absolute top-3 left-3 rounded-xl px-3 py-1 text-sm font-black"
                                    >
                                        #{{ $index + 1 }}
                                    </div>
                                </div>
        ...
        {{-- <section class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <p class="mt-1 text-sm text-slate-500">Response lengkap dari FastAPI rekomendasi.</p>
